  ...
  function generateDirectionHtml(direction, context) {
    ...
    if (isFullView) {
      allDayTypes.forEach(dayType => {
        directionHtml += `<p class="mt-6 mb-2 text-gray-700">${dayType.displayName}</p>
                                  ${generateTimesGridHtml({ timesArray: direction.times[dayType.key] || [], notesLegend, isShortView: false, now, dayTypeForGrid: dayType })}`;
      });
    } else {
      directionHtml += `<div class="flex justify-between items-center mt-2 mb-2"><p class="text-gray-700">Najbliższe kursy dzisiaj (${today.displayName})</p></div>
    ...
    if (Object.keys(notesLegend).length > 0) {
      ...
      for (const key in notesLegend) legendHtml += `<p class="text-sm text-gray-600"><span class="font-bold">${key}</span> - ${notesLegend[key]}</p>`;
      legendHtml += '</div>';
    }
